...
from options.train_ssl_options import TrainOptions
import os
import numpy as np
...
def main():
    ...
    for i in range(start_iter, start_iter + total_num):
        ...
        #import pdb;pdb.set_trace()    
        try:
            src_img, src_lbl, _, name = sourceloader_iter.next()
        except StopIteration:
            sourceloader_iter = iter(sourceloader)
            src_img, src_lbl, _, name = sourceloader_iter.next()
        src_img, src_lbl = Variable(src_img).cuda(), Variable(src_lbl.long()).cuda()
        #import pdb;pdb.set_trace()
        src_seg_score1, src_seg_score2, src_seg_score3, src_seg_score4 = model(src_img, lbl=src_lbl)
       ...
       #     train_writer.add_scalar(m, eval(m), i+1)
        ...
        if (i+1) % args.print_freq == 0:
            ...
            print ('[it %d][src seg loss %.4f][trg loss %.4f][lr %.4f][%.2fs]' % \
                    (i + 1, loss_seg_src.data,loss_trg.data,optimizer.param_groups[0]['lr']*10000, _t['iter time'].diff))
        ...
        if i + 1 == start_iter + total_num and load_selected_samples is None:
            ...
            noisy_ind_update = ind_sorted[num_remember:]
            clean_name_update = np.array(name_list)[clean_ind_update]
            noisy_name_update = np.array(name_list)[noisy_ind_update]
           ...
           # with open(os.path.join(args.save_selected_samples), "a") as f:
           #     for i in range(len(noisy_name_update)):
           #         f.write(str(noisy_name_update[i][0]) + '\n')
        ...
        #import pdb;pdb.set_trace()
        src_lbl = torch.unsqueeze(src_lbl,0)
        src_lbl = nn.functional.upsample(src_lbl.float(),size= (2056, 2124),mode= 'bilinear', align_corners=True).cpu().data[0].numpy().squeeze()
        noise_disc_dice, noise_cup_dice = calculate_dice(args.data_dir, src_lbl, name)
        ...
        with open("nosie_0.9_dice2.txt", "a") as f:
            f.write(name[0])
            f.write('===>predict disc_dice:' + str(round(predict_disc_dice, 3)) + '\t' + 'cup_dice:' + str(round(predict_cup_dice, 3))+ '\n')
            f.write('===>noise_included disc_dice:' + str(round(noise_disc_dice, 3)) + '\t' + 'cup_dice:' + str(round(noise_cup_dice, 3)) + '\n')

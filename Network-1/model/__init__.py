...
def CreateDiscriminator(args, s):
    ...
    if args.restore_from is not None:
        if s == 1:
            discriminator.load_state_dict(torch.load(args.restore_from + '_D1.pth'))
        if s == 2:
            discriminator.load_state_dict(torch.load(args.restore_from + '_D2.pth'))

...
def Generator_Gene(data_list, batch_size, train_path, img_size, CDRSeg_size=400, pt=False, phase='train'):
    #try:
    ...
    train_ids = [i_id.strip() for i_id in open(data_list)]
    #train_ids = [i_id.strip('\t')[0:5] for i_id in open(load_selected_samples)]
    #import pdb;pdb.set_trace()
    print(len(train_ids))
    X = np.zeros((len(train_ids), img_size, img_size, 3), dtype=np.uint8)
    Y = np.zeros((len(train_ids), img_size, img_size, 1), dtype=np.uint8)
    for i, id_ in enumerate(train_ids):
        path = os.path.join(train_path, 'image/', id_.split('.')[0] + ".png")
        img = np.asarray(image.load_img(path))
        X[i] = img
        mask_ = np.asarray(
    ...
    while True:
        ...
        while batch_index < batch_size:
            ...
            if index == len(train_ids):
                ...
                #random.shuffle(index_list)
        X_train = imagenet_utils.preprocess_input(X_train.astype(np.float32), mode='tf')
        yield X_train, Y_train, mask_, train_ids[index_list[index-1]].split('.')[0]

...
def calculate_dice(mask_path, prediction_path):
    ...
    for filename in file_test_list:
        ...
        prediction = np.asarray(image.load_img(os.path.join(prediction_path, filename.split('.')[0]+'.png'), grayscale=True))
...
if __name__ == '__main__':
    ...
    load_from = "./weights0/" + dataset + "/DA_patch/Generator/generator_150.h5"
   ...
   # evaluate_segmentation_results(data_save_path, mask_path,ext='png', output_path='./')
    ...
    print('disc_dice:',disc_dice, 'cup_dice:',cup_dice)
    print(load_from)
